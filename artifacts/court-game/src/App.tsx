...
const HELP_TOPICS_SOURCE: HelpTopicDraft[] = [
  ...
  },
  {
    ...
    content:
      "Это совокупность обязательных признаков, по которым деяние считается преступлением. Включает объекты, субъекты, субъективную и объективную сторону преступления. Если хотя бы одного элемента нет, обвинение становится слабым или необоснованным.",
    keywords: ["Состав", "Состав преступления", "преступление"],
  },
  {
    id: "term-pleading",
    category: "Термины",
  ...
  if (screen === "game" && game && game.me) {
    ...
    const latestUsedCardId =
      game.usedCards.length > 0
        ...
        : null;
    const preparationStageIndex = gameStages.indexOf("Подготовка");
    const openingSpeechStageIndex = gameStages.indexOf("Выступление истца");
    const resolvedPreparationStageIndex =
      preparationStageIndex >= 0 ? preparationStageIndex : 0;
    const resolvedOpeningSpeechStageIndex =
      openingSpeechStageIndex >= 0 ? openingSpeechStageIndex : 1;
    const isPreparationStage = game.stageIndex === resolvedPreparationStageIndex;
    const isOpeningSpeechStage =
      game.stageIndex === resolvedOpeningSpeechStageIndex;
    const openingSpeechRevealedFacts = game.revealedFacts.filter(
      (fact) => fact.stageIndex === resolvedOpeningSpeechStageIndex,
    ).length;
